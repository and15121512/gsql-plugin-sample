...
bad_func_stmt: T_ASYNC? L_ID T_OPEN_P other;

//bad_func: func_start func_mid func_end #bad

other_stmt: other (T_ON choose_engine)? T_SEMICOLON;

other:
    ( var | interpolation_expr | string | T_OPEN_P other T_CLOSE_P | ~(T_DOLLAR) )*?
;
...
try_catch_stmt:
     T_TRY try_bock=block T_CATCH (exc=L_ID T_THEN)? catch_block=block T_END
     ;
...
choose_engine:
     T_ENGINE (L_ID || expr) engine_params?
     ;

engine_params:
     T_OPEN_P L_ID T_EQUAL expr (T_COMMA L_ID T_EQUAL expr)* T_CLOSE_P
     ;
...
assigment_stmt:
       T_LET T_GLOBAL? L_ID T_EQUAL (T_CURSOR T_IS)? expr T_SEMICOLON                      #assigment_default
     | T_LET T_GLOBAL? L_ID T_OPEN_SB index=expr T_CLOSE_SB T_EQUAL value=expr T_SEMICOLON #assigment_by_index
     | T_LET T_GLOBAL? L_ID (T_COMMA L_ID)* T_EQUAL expr (T_COMMA expr)* T_SEMICOLON      #assigment_multiple
     ;

open_cursor_stmt:
    T_OPEN L_ID T_SEMICOLON;

close_cursor_stmt:
    T_CLOSE L_ID T_SEMICOLON;
...
for_cursor_stmt :       // FOR (cursor) statement
       T_FOR L_ID (T_COMMA L_ID)* T_IN (T_CURSOR T_IS)? expr T_LOOP block T_END T_LOOP
     ;

for_range_stmt :        // FOR (Integer range) statement
       T_FOR L_ID T_IN T_REVERSE? from=expr T_DOT2 to=expr ((T_BY | T_STEP) step=expr)? T_LOOP block T_END T_LOOP
     ;
...
expr: // TODO other expressions if needed
       lambda                                                    #expr_lambda
     | T_FETCH L_ID                                             #expr_fetch_cursor
...
//     | T_INTERVAL expr interval_item                             #expr_interval // TODO do we need it? if need its literal
     | case_r                                                    #expr_case
//     | L_ID T_PERCENT (T_ISOPEN | T_FOUND | T_NOTFOUND)         #expr_found // TODO do we need it?
...
//func:
//     T_ASYNC? L_ID T_OPEN_P (arg (T_COMMA arg)*)? T_CLOSE_P
//     ;
func:
     T_ASYNC? L_ID T_OPEN_P (arg (T_COMMA arg)*)? T_CLOSE_P #goodFunc
     | T_ASYNC? L_ID T_OPEN_P (T_COMMA* arg (
        (T_COMMA? arg) | (T_COMMA arg?)
     )*
     )?  T_CLOSE_P #argsUnrecFunc
     ;
...
lambda:
     T_OPEN_P L_ID? (T_COMMA L_ID)* T_CLOSE_P T_LABMDA T_BEGIN block T_END
     ;
...
//L_ID: simple_name (('.' | T_COLON) simple_name)*;
//
//simple_name:
//       L_NAME
//     | non_reserved_words
...
string:                                   // String literal (single or double quoted)
       R_S_QUOTE raw_s_string T_RS_CLOSE_QUOTE    #raw_single_quotedString
     | R_B_QUOTE raw_b_string X_RB_CLOSE_QUOTE    #raw_slash_quotedString
     | R_D_QUOTE raw_d_string T_RD_CLOSE_QUOTE    #raw_double_quotedString
...
non_reserved_words :                      // Tokens that are not reserved words and can be used as L_IDifiers
       T_ACTION
     | T_ACTIVITY_COUNT
     | T_ADD2
     | T_ALL
